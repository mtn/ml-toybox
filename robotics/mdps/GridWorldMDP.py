...
class GridWorldMDP(object):
    ...
    def __init__(self, noise=0.2, gamma=0.9):
        ...
        )  # +1 for the extra absorbing state
    ...
    def valueIteration(self, epsilon):
        ...
        Pi = np.zeros(
            [self.numstates]
        )  # Policy where Pi[i] is 0 (N), 1 (E), 2 (S), 3(W)

        n = 0  # Keep track of the number of iterations

        while True:
            # Update the values and policy
            rewards = self.R + self.gamma * V[np.newaxis, :, np.newaxis]
            tmp = np.einsum("ijk,ijk->ik", self.T, rewards)
            V_new = np.max(tmp, axis=1)
            Pi = np.argmax(tmp, axis=1)

            n += 1

            # Check for convergence
            if np.max(abs(V - V_new)) <= epsilon:
                V = V_new
                break
