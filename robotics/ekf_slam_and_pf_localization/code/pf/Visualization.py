...
class Visualization(object):
        ...
        for j in range(n):
            for i in range(m):
                ...
                if gridmap.inCollision(j, i, True):
                    self.ax.add_patch(
                        patches.Rectangle((x0, y0), gridmap.xres, gridmap.yres)
                    )
    ...
    def drawLidar(self, range, bearing, x, y, theta):
        ...
        XY_world = (
            R.dot(XY_lidar) + XY_robot
        )  # np.tile(np.array([[x],[y]]),(1,bearing.shape[0]))
        ...
        if self.lidar_handle == None:
            ...
            #     self.line_handle,
            self.lidar_handle = LineCollection(
                lines, cmap=plt.cm.gist_ncar, linewidths=0.5, color="red"
            )
